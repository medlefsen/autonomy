    ...
    if(selected_object.is_valid())
      {
        ...
        std::string fuel;
        if(typeid(*selected_object) == typeid(scripted_drone))
          {
            unit_type = "Drone";
            fuel = boost::lexical_cast<std::string>(static_cast<drone&>(*selected_object).get_fuel());
          }
        if(typeid(*selected_object) == typeid(asteroid))
          { 
            unit_type = "Asteroid";
            fuel = boost::lexical_cast<std::string>(static_cast<asteroid&>(*selected_object).get_fuel());
          }    
        if(typeid(*selected_object) == typeid(base_station))
          {
