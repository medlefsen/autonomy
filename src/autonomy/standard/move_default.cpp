...
BOOST_CLASS_EXPORT(autonomy::standard::move_direction_default);
BOOST_CLASS_EXPORT(autonomy::standard::move_direction_response_default);

namespace autonomy { namespace standard
{
  void move_direction_default::execute( universe & entity )
  {
    ...
    for(auto& md : _action_group)
    {
      ...
      if (loc.query(new_loc) == entity_id_t())
      {
        ...
        mdr = action::create<move_direction_response>(true);
      }
      else
      {
        mdr = action::create<move_direction_response>(false);
      }
      (md->subject())->send_action(mdr);
    }
  }

  void move_direction_response_default::execute( scripted_drone & entity )
  {
